...
import {
  GenericSelctionOption,
  Sector,
} from "../modals/portfolioConstruction/types";
import { FactorDialogEquityProps } from "./overrideConstructionConfig.tsx";
import {
  BootStrappedStack,
  TableContainer,
  getKeyByValue,
} from "../modals/portfolioConstruction/Constraint/constraintDialogTP.tsx";
import { enrichConstraints } from "../modals/portfolioConstruction/lib.ts";
...
const FactorConstraintsContentEquity: React.FC<FactorDialogEquityProps> = ({
  constraintTypes,
  optimizationDefinition,
  esgModels,
  riskModels,
  ...
  useEffect(() => {
    console.log("here");
    setFilteredModels(esgConstraintsArray);
  }, []);
  ...
  useEffect(() => {
    ...
    const constructed = constuctConstraints(
      optimizationDefinition?.constraints!
    );
  ...
  const updateFilteredModels = () => {
    ...
    if (firstMissingModel) {
      ...
      setOptimizationDefintion(dup);
      console.log([...filteredModels, newModel]);
...
export default FactorConstraintsContentEquity;
